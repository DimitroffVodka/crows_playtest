...
3. ~~**Per-target modifiers have no input channel.**~~ — **DECIDED by D3, 2026-08-23. Route: CODE LATER, in a dedicated post-T2.2 ticket. Plus ONE thing to do in this wave.**
   ...
   ### ⚠ This is a live correctness bug that silently turns a miss into a hit
   **Do this now, before the full interface:** `attack.mjs:202` loops `labels.warnings` into **`console.warn` only**. It is absent from `TestResult`, `testCardData`, the template and `ui.notifications`. The player sees a confident, ordinary target tier that is wrong. D3 ran the pure path: raw 12, target A clear, target B in cover — current code gives both **T2 and deals T2 damage to B**, where resolving B's already-built cover bane gives **T1**. **Make the warning a player-visible refusal** (`ui.notifications`, no roll, no card) until the real seam exists. A loud refusal beats a false hit. Worse, the ordinary sheet caller supplies only conditions and no geometry, so many attacks omit cover/range and produce **no warning at all**.

   **The register's premise was false.** "Only the input signature lags" holds *only* for target **conditions on weapon attacks**. Verified trace: `targetLabels()` (`combat.mjs:162-218`) correctly builds per-target cover/range, `buildAttackLabels()` leaves them in `labels.targets[]` on multi-target, and then **`attack.mjs:210-220` discards them**, passing only `targetRef(tokenId, conditions)`. `rollTest` never sees cover, flanking, high ground, concealment or range. Additionally **`TestResult.targets[]` carries no numeric mods at all** — only edges/banes/tier/terminal — so the claim is literally false for the range penalty before any downstream behaviour.
