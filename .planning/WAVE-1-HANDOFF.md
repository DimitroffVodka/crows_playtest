...
3. ~~**Per-target modifiers have no input channel.**~~ — **DECIDED by D3, 2026-08-23. Route: CODE LATER, in a dedicated post-T2.2 ticket. Plus ONE thing to do in this wave.**
   ...
   **Adjacent follow-up found by mutation testing — NOT fixed here:** the Fury block in `attack.mjs` wraps both `import("./crypt.mjs")` **and the whole `consumeBoonOnDamage()` call** in a bare `catch`, then continues the attack. A deterministic sentinel saw `1d6` fail inside Fury and the code proceed to `2d10`; an update/chat failure can therefore also be swallowed after partial mutation. Give this its own refusal/rollback ticket rather than widening this narrowly agreed patch.
